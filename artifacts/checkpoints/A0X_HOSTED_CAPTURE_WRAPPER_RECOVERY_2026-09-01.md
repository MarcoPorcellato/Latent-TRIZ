...
- current capture implementation commit: `e233c6012b18ec34ae5501799261e8bed52d1893`;
- current capture implementation tree: `0cc1d86c7d56dd5b3fd0895de5a64cea93f6e3e5`.
...
3. Output transaction is absent-only, no-overwrite, inode-owned staging cleanup, and rechecks the expected child bytes/digests through the held stage descriptor both before and after exclusive publication. It proves the held identity again immediately before `rmdir`; parent-chain discontinuity after publication becomes `A0X_HOSTED_CAPTURE_PUBLICATION_OWNERSHIP_LOST` without deleting a foreign visible path.
...
Aggregate and inventory source changes are complete at the implementation commit above. Transport timestamps require `created_at <= captured_at < expires_at` before capture admission. The adapter's 30-second timeout and per-operation stdout caps are complete. Tracked generated inventories, freezes, dossiers, and no-model receipt are intentionally stale. Regeneration requires separate exact-head authorization; do not regenerate. The current stop is fresh exact-head security review only. No real GitHub, attestation verification, Gate B/C, model, tokenizer, target, CCP, Docker, push, PR, or merge is authorized.
